...
카테고리가 `현장사례`인 글은 아래 항목을 예외 처리한다.

| # | 예외 조건 | 이유 |
|---|----------|------|
| 3 | `cases/[폴더]/photo*.jpg` 가 있으면 현장사진 조건 통과 | 납품사진이 현장사진 역할을 대체 |
| 4 | 납품사진(블러 없음)은 EXIF 검사 건너뜀 | 블러 처리 안 했으므로 EXIF 보정 불필요 |
| 5 | 블러 안 한 경우 펌프좌표 검사 건너뜀 | blur-config.json 없음 = 납품사진 사용 중 |
