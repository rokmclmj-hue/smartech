...
GC(질량분석장비) 같은 분석 장비와 동결건조·멸균 챔버 같은 공정 장비는 오일 유무를 판단하는 기준이 다릅니다. 분석 장비는 소형 오일로터리펌프가 제조사 출고 시부터 붙박이로 달려 있어 전환이 쉽지 않고 꼭 필요하지도 않은 반면, 공정 장비는 제품이 배기 라인과 접촉할 가능성이 있어 오일 역류가 곧 오염·규제 위반으로 이어질 수 있습니다. 이 글은 두 장비군을 나눠 각각의 판단 기준을 정리합니다.

![의료·생명공학 진공 시스템 무오일 드라이펌프 선택 기준](./images/thumbnail.png)

## GC(질량분석장비) 같은 분석 장비, 오일로터리가 표준인 이유

질량분석기·전자현미경·GC 계열 장비는 E2M0.7(배기속도 0.75 m³/h, 도달진공도 3.0E-03 mbar, NW10)·E2M1.5(1.6 m³/h, 1.0E-03 mbar, NW25) 같은 초소형 오일로터리베인 펌프를 기본 사양으로 채택합니다. 모델별로 입구 규격은 다르지만 장비 내부 공간에 맞춰 설계된 소형 펌프라는 점은 공통이고, 터보펌프 백킹용으로도 널리 쓰입니다.

이 펌프들은 장비 제조사가 애초에 이 크기·성능에 맞춰 시스템을 설계했기 때문에, 다른 펌프로 단순 교체하기가 현실적으로 어렵습니다. 배관 경로, 진동 허용치, 설치 공간까지 전부 E2M 시리즈 기준으로 맞춰져 있어서, 드라이펌프로 바꾸려면 배관을 새로 설계하거나 최악의 경우 장비 자체를 재설계해야 하는 상황도 생깁니다. 그래서 GC(질량분석장비)를 운용 중인 현장에서는 "오일식이라 문제"보다 "오일식이 기본"이라는 인식이 맞습니다.

![GC(질량분석장비) 배기 라인의 오일로터리펌프](./images/사진1.png)
...
드라이 대안이 아예 없는 것은 아닙니다. nXDS6i(드라이 스크롤, 6.2 m³/h, 2.0E-02 mbar, NW25)는 E2M 시리즈와 동일하게 질량분석기·전자현미경·터보펌프 백킹용으로 쓰이는 오일프리 모델입니다. E2M0.7(NW10)을 쓰던 곳은 입구 규격이 바뀌어 배관을 다시 맞춰야 하는 경우가 많고, E2M1.5(NW25)를 쓰던 곳은 입구 규격은 같아도 배기속도·설치 공간 차이는 별도로 확인해야 합니다. 신규 장비 도입이나 실험실 리모델링 시점에는 검토할 만한 선택지입니다. 다만 기존 GC를 그대로 쓰면서 펌프만 바꾸는 방식은 대부분 권장하지 않습니다.
...
![오일 역류 경로와 멸균 공정 오염 위험](./images/사진2.png)
...
![동결건조 챔버 응축기 진공펌프 구성도](./images/사진3.png)

동결건조 같은 공정 장비에서는 스크류 방식 드라이펌프가 자주 채택됩니다. 100% 오일프리로 마모 부품이 없어 오일 교환·부품 교체 부담이 적습니다. 오일 상태를 점검하거나 교환할 필요가 없어 배치 종료 직후 바로 다음 배치를 가동할 수 있고, 이 덕분에 유지보수로 인한 대기시간이 줄어듭니다. 반복 배치를 많이 돌리는 라인일수록 이 이점이 크게 체감됩니다.

![스크류 드라이펌프 오일프리 유지보수 흐름](./images/사진4.png)
...
- **GC(질량분석장비)·전자현미경 같은 분석 장비**: E2M0.7·E2M1.5급 오일로터리 유지가 기본입니다. 오일 냄새·미스트가 신경 쓰인다면 EMF3 필터 교체 주기부터 점검하고, 신규 도입 시점이라면 nXDS6i 같은 드라이 스크롤을 검토할 수 있습니다.
