...
**연구실 진공 배관에서 KF는 소형(NW10~50) 배관에, ISO-K는 터보펌프 메인 인렛(DN63 이상)에, CF는 초고진공(UHV) 환경에 씁니다.** 세 규격은 크기·진공 등급·조립 방식이 모두 달라, 연결 부위의 크기와 목표 진공도를 먼저 확인한 뒤 선택해야 합니다.
...
## KF·ISO-K·CF 플랜지의 차이는 무엇인가?

진공 배관 피팅은 세 가지 기준으로 나뉩니다. **어디에 연결하느냐(크기)**, **얼마나 깊은 진공이 필요하느냐(진공 등급)**, **얼마나 자주 분리하느냐(조립 방식)** — 이 세 가지를 먼저 정하면 플랜지 규격이 자연스럽게 결정됩니다.

- **KF(NW)** (별칭 QF, Klein Flange): 크기 NW10~NW50, 도달 진공도 ~10⁻⁸ mbar, 클램프 1개 + O링으로 조립
- **ISO-K**: 크기 DN63~DN500, 도달 진공도 ~10⁻⁹ mbar, 클램프 또는 볼트로 조립
- **CF**(ConFlat): 크기 CF35~CF200, 도달 진공도 ~10⁻¹² mbar, 볼트 + 금속 개스킷으로 조립
...
- **nEXT85**: 메인 인렛 DN63 ISO-K 또는 DN100 ISO-K, 백킹 포트 NW16
- **nEXT240 / nEXT300**: 메인 인렛 DN100 ISO-K, 백킹 포트 NW25
- **nEXT400D**: 메인 인렛 **DN160 ISO-K**, 백킹 포트 NW25
- **nEXT730D**: 메인 인렛 DN160 ISO-K, 백킹 포트 NW25
- **nEXT930D**: 메인 인렛 DN200 ISO-K, 백킹 포트 NW25
...
## 규격이 다른 배관을 연결할 때 어떤 어댑터를 써야 하나?
...
KF·ISO-K 어댑터·센터링 링·클램프 국내 재고 보유. nEXT 터보펌프 도입 시 기존 배관 규격 알려주시면 어댑터 구성 제안 가능.
