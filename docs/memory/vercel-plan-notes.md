...
## 대표님 결정 (2026-09-25): 당분간 무료(Hobby) 유지

- 이유: 현재 사용량이 무료 한도 안이고, 약관 문제는 보통 이메일로 먼저 연락이 온다. 업그레이드는 몇 분이면 된다.
- **전환 시점(아래 중 하나라도 생기면 Pro로 전환)**
  1. **홈페이지에 카드 결제(토스페이먼츠) 연결할 때 — 필수.** 약관의 "방문자에게 결제 요청"이 명시적 상업 사용이다.
  2. Vercel 사용량 100% 알림
  3. Vercel의 상업적 사용 안내·정지 메일
  4. 광고·이벤트로 방문자 급증 예상
- 대표님 할 일: rokmclmj@gmail.com의 Vercel 메일 제목 확인(usage·paused·commercial).
- 사이트가 갑자기 멈추면: Vercel 로그인 → Pro 업그레이드 → 정지 사유가 약관이면 지원팀 연락. 한도 초과 정지는 프로젝트 화면에서 Resume.
