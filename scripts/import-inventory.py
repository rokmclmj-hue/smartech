"""
재고 품목 초기 임포트 스크립트
사용법: python scripts/import-inventory.py
"""
import urllib.request, json, sys, os

sys.stdout.reconfigure(encoding="utf-8")

ENV_PATH = os.path.join(os.path.dirname(__file__), "..", "블로그", ".env")
...
BASE_URL = env.get("API_URL", "https://www.smartechvacuum.com").rsplit("/api/", 1)[0]
API_URL = f"{BASE_URL}/api/admin/inventory"

if not SECRET:
    print("[ERROR] .env에 BLOG_UPLOAD_SECRET 없음")
    sys.exit(1)
...
print(f"총 {len(ITEMS)}개 품목 임포트 시작…\n")
ok = 0
fail = 0
...
for item in ITEMS:
    ...
    try:
        with urllib.request.urlopen(req) as res:
            ...
            print(f"  ✅ [{item['department']}] {item['partNo']} — {item['name'][:40]}")
            ok += 1
    except urllib.error.HTTPError as e:
        body = e.read().decode("utf-8", errors="replace")
        if "Unique constraint" in body or "already exists" in body:
            print(f"  ⏭  [{item['department']}] {item['partNo']} — 이미 존재 (건너뜀)")
        else:
            print(f"  ❌ [{item['department']}] {item['partNo']} — HTTP {e.code}: {body[:80]}")
            fail += 1
    except Exception as e:
        print(f"  ❌ {item['partNo']} — {e}")
        fail += 1
