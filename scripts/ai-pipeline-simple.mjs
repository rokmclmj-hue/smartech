...
async function main() {
  ...
  const topicRaw = await ask(
    `당신은 스마텍 블로그 편집장입니다. 고객 이메일과 상담 기록을 보고 오늘 블로그에 올릴 이슈 1개를 선정합니다.
출력 형식 (반드시 아래 형식만):
주제: [이슈 한 줄]
카테고리: [수리문의 / 견적문의 / 기술문의 / 단종문의 / 일반 중 1개]
표제: [* 모델명 + 업무유형 * 형식, 예: * RV8 수리 * 또는 * nXDS15i 납품(수원) *]`,
    `[고객 이메일 최근 30건]\n${emails}\n\n[상담 통화 기록]\n${calls}\n\n오늘 블로그에 올릴 이슈 1개를 선정하세요.`
  , '주제선정');
...
- 첫 줄: 표제만 단독으로 (예: * RV8 수리 *)
- 표제 다음 줄: 본문 바로 시작
- 구체적 모델명, 날짜, 수치 포함
- "탁월한", "최고의" 등 과장 표현 금지
- 고객사 실명 금지 (영문 이니셜도 금지 — "고객사" 또는 "현장"으로만)
