...
async function main() {
  const candidate = await prisma.xPost.findFirst({
  ...
  if (weighted > 280) {
    log(`[건너뜀] id=${candidate.id} — X 기준 글자 수 초과(${weighted}/280자). 관리자 확인 필요.`);
    await prisma.$disconnect();
    return;
  }
  ...
  if (claim.count === 0) {
    log(`[건너뜀] id=${candidate.id} — 이미 다른 프로세스가 처리 중`);
    await prisma.$disconnect();
    return;
  }
  ...
  try {
    ...
    if (e instanceof PostedButUnconfirmedError) {
      ...
      log(`[확인 필요] id=${candidate.id} — ${e.message} — PUBLISHING 상태로 잠김, 관리자가 X 계정에서 직접 확인 필요`);
    } else {
      await prisma.xPost.update({ where: { id: candidate.id }, data: { status: "APPROVED" } });
      log(`[ERROR] id=${candidate.id} 게시 실패 — ${e instanceof Error ? e.message : String(e)} — APPROVED로 되돌림, 내일 재시도`);
    }
  }
